...
import { CustomSidebarView } from './CustomSidebarView';
import * as path from 'path';
import * as fs from 'fs';
import { v4 as uuidv4 } from 'uuid';
import * as dotenv from 'dotenv';
import axios from 'axios';
import pool from './database'; // นำเข้า pool ที่ export จาก database.ts
...
let currentToken = '';
let currentUsername = '';
let statusBar: vscode.StatusBarItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 100); 
let initialWordCount: number = 0;
let isAfk: boolean = false;
let lastActiveTime: number = 0;
let afkTimeLimit: number = 300000; // 5 minutes in milliseconds
let totalActiveTime: number = 0;
let afkTimeoutId: NodeJS.Timeout | null = null;
let currentLanguage: string = 'JavaScript'; // กำหนดค่าเริ่มต้นให้กับ currentLanguage
...
function readTokenFromFile(): { uid: string, token: string } | null {
...
function writeTokenToFile(uid: string, token: string): void {
...
async function updateDatabase(uid: string, token: string, isLoggedIn: boolean) {
  const client = await pool.connect(); // ใช้ pool ที่นำเข้ามาจาก database.ts
  try {
...
async function getUsernameByUid(uid: string): Promise<string | null> {
  const client = await pool.connect(); // ใช้ pool ที่นำเข้ามาจาก database.ts
  try {
...
async function validateTokenInDatabase(uid: string, token: string): Promise<boolean> {
  const client = await pool.connect(); // ใช้ pool ที่นำเข้ามาจาก database.ts
  try {
...
async function searchGoogleAndSaveResults(content: string, projectName: string) {
...
async function saveCodeReferencesToDatabase(activityCodeID: string, projectName: string, links: string, pasteCount: number) {
  const client = await pool.connect(); // ใช้ pool ที่นำเข้ามาจาก database.ts
  try {
    const sqlQuery = `
...
function saveToDatabase(data: {
  ...
  languages: string;
  wordcount: number;
  coins: number; // Ensure this is a number
  time: string;
  uid: string;
  projectName: string;
}) {
...
class ActivityHandler {
  static changeTextListener: vscode.Disposable | null = null;
...
class TimerManager {
  static resetAfkTimeout() {
    ...
    let afkSeconds = 0;
    let afkIntervalId: NodeJS.Timeout | undefined;
    ...
    }, afkTimeLimit) as NodeJS.Timeout;
  }

  static startTimer(isStart: boolean) {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const customSidebarProvider = new CustomSidebarView(context.extensionUri, CustomSidebarView.viewType);
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(
      CustomSidebarView.viewType,
      customSidebarProvider
    )
  );

  const customLoginProvider = new CustomSidebarView(context.extensionUri, CustomSidebarView.loginViewType);
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(
      CustomSidebarView.loginViewType,
      customLoginProvider
    )
  );
  ...
  const logout = vscode.commands.registerCommand('pawpals.logout', () => {
    ...
    const customSidebarProvider = new CustomSidebarView(context.extensionUri, CustomSidebarView.loginViewType);
    context.subscriptions.push(
      vscode.window.registerWebviewViewProvider(
        CustomSidebarView.loginViewType,
        customSidebarProvider
